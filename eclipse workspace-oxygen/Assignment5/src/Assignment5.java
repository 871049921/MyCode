...
public class Assignment5 extends Application {
	...
	public void start(Stage primaryStage) {
		...
		VBox vBox1 = new VBox();
		VBox vBox2 = new VBox();
		Label lbFloor = new Label("当前楼层数为：1");
		Label lbNumber = new Label("当电梯人数为：0");
		...
		btStart.setOnAction(e -> {
			...
			new Thread(new Runnable() {// 借鉴网上的

				private boolean isStationary = false;// 默认不执行wait, 即暂停的

				public synchronized void toStation() {// 同步方法
					isStationary = true;
				}

				public synchronized void toMove() {// 不执行wait，并唤醒暂停的线程
					isStationary = false;
					...
					try {
						for (; output1[i] != null; i++) {
							...
							Platform.runLater(new Runnable() {
								@Override
								public void run() {
									lbFloor.setText("当前楼层数为：" + output1[i].split(",")[0]);
									if (output[i].contains("FR")) {
										numberInElevator++;
									} else if (output[i].contains("ER") && numberInElevator != 0) {
										numberInElevator--;
									}
									lbNumber.setText("当电梯人数为：" + numberInElevator);

								}
							});

							Thread.sleep(2500);
							synchronized (this) {// 借鉴网上的
								while (isStationary) {
									try {
										wait();// 线程进入等待状态
