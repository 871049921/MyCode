...
public class Experiment3 {
	...
	public static void show(ArrayList<AnimalShow> objectList) {
		...
		String stdTime = transformMinuteIntoHour(currentMinute);
		System.out.println(stdTime + "   下一个出场的是" + "美人鱼，现在有请它出场");
		Mermaid mermaid = new Mermaid("pw");
		ShowItem showItem[] = mermaid.getShowItem();
		Interaction interaction[] = mermaid.getInteraction();
		for (int j = 0; j < 2; j++){
			stdTime = transformMinuteIntoHour(currentMinute);
			int num = selectRandomShowOrInteraction();
			System.out.println(stdTime + "   " + mermaid.getName() + "表演" + showItem[num].getName());
			currentMinute += showItem[num].getTimeOfShow();
		}
		int num = selectRandomShowOrInteraction();
		stdTime = transformMinuteIntoHour(currentMinute);
		System.out.println(stdTime + "   现在我们征选" + (int)(Math.random() * 10 + 1) + "位幸运观众跟" + mermaid.getName() + interaction[num].getName());
		currentMinute += interaction[num].getTime();
